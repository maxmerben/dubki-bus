        ...
        if not place:
            ...
            if place:
                if (not weekday) and (not time):
                    ...
                    reply = reply + f"Рейсы от {places_rus_names_list[place]['gen']} на {nullize(time)} сегодня:"
                else:
                    reply = reply + f"Рейсы от {places_rus_names_list[place]['gen']} " \
                                    f"на {nullize(time)} в {weekdays_rus_names_list[weekday]['acc']}:"
...
def get_next_bus(message, place=False, weekday=False, time=False, reply=False):
    ...
    if not place in places_list:
        ...
        if not suggested_buses:
            bot.send_message(message.chat.id, f"К сожалению, в это время в {weekdays_rus_names_list[weekday]['acc']} "
                                              f"от {places_rus_names_list[place]['gen']} автобусы не идут.",
                             reply_markup=types.ReplyKeyboardRemove())
            return
